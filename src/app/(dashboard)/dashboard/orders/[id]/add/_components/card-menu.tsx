...
export default function CardMenu({ menu, onAddToCart }: { menu: Menu; onAddToCart: (item:Menu, type:'dec'|'inc')=>void;}) {
    return (
        <Card key={menu.id} className="w-full h-fit border shadow-sm p-0 gap-0">
            <Image src={`${menu.image_url}`} alt={menu.name} width={300} height={300} className="w-full h-[200px] object-cover rounded-t-lg" />
            ...
            <CardFooter className="flex justify-between items-center p-4">
                <div className="text-xl font-bold">{IDR(menu.price)}</div>
