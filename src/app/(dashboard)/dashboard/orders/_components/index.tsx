...
import { Ban, Link2Icon, Loader2, ScrollText, Trash } from "lucide-react";
...
export default function OrderManagement() {
  ...
  }, [statusState, refetch, refetchTables]);
...
const filterData = useMemo(() => {
  ...
  return (
    orders?.data?.map((item, index) => [
      ...
      item.order_id,
      item.customer_name,
      (item.tables as unknown as { name: string }).name,
          ...
          ...(item.status === "reserved"
            ? statusActionList.map((row) => ({
                ...
                action: () =>
                  row.action(
                    item.id,
                    (item.tables as unknown as { id: string }).id
                  ),
                type: row.type,
                variant: row.variant,
              }))
      ...
      <div className="flex flex-col mb-4 gap-4 justify-between w-full lg:flex-row">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Dialog>
            ...
            <CreateOrder tables={tables} refetch={refetch} />
          </Dialog>
        </div>
      </div>
